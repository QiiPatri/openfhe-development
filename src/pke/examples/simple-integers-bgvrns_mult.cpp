// Derived from simple-integers-bgvrns.cpp - mult timings
#include "openfhe.h"
...
int main() {
    ...
    Ciphertext<DCRTPoly> cMul12, cMulRes, cScalarMul;
    int64_t scalar = 5;
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        auto t1 = std::chrono::high_resolution_clock::now();
        cScalarMul = cryptoContext->EvalMult(c1, scalarP);
        auto t2 = std::chrono::high_resolution_clock::now();
        scalar_mult_sum += std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count();
    }

    // std::cout << "OPENFHE/BGV/密文-密文乘法 平均: " << mult_sum / 10 << " us" << std::endl;
    // std::cout << "OPENFHE/BGV/密文-明文乘法 平均: " << scalar_mult_sum / 10 << " us" << std::endl;
    std::cout << "OPENFHE/BGV/密文-密文乘法 总耗时(us): " << mult_sum << ", 执行次数:" << 10 << ", 平均耗时(us): " << mult_sum / 10 << std::endl;
    std::cout << "OPENFHE/BGV/密文-明文乘法 总耗时(us): " << scalar_mult_sum << ", 执行次数:" << 10 << ", 平均耗时(us): " << scalar_mult_sum / 10 << std::endl;
    return 0;
}
