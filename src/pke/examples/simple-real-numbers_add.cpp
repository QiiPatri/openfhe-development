...
int main() {
    ...
    std::cout << "OPENFHE/CKKS/密文-密文加法 总耗时(us): " << add_sum << ", 执行次数:" << 10 << ", 平均耗时(us): " << add_sum / 10 << std::endl;
    std::cout << "OPENFHE/CKKS/密文-明文加法 总耗时(us): " << addconst_sum << ", 执行次数:" << 10 << ", 平均耗时(us): " << addconst_sum / 10 << std::endl;
    return 0;
}
